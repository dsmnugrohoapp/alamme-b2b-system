...
export default function OrderForm({
  ...
  return (
    <form action={saveOrder}>
      ...
      {(order?.shipping_preference || order?.payment_method_preference) && (
        <div className="bg-blue-50 border border-blue-100 text-blue-800 text-xs rounded-lg px-3.5 py-2.5 mb-3">
          📩 Order mandiri dari customer — preferensi kurir: <b>{order.shipping_preference || '-'}</b>, preferensi pembayaran: <b>{order.payment_method_preference || '-'}</b>.
        </div>
      )}
      ...
      <fieldset className="border border-dashed border-gray-300 rounded-lg p-3 mb-3">
        ...
        <div className="hidden md:grid grid-cols-[2fr_70px_100px_170px_110px_28px] gap-2 text-[10.5px] uppercase text-gray-500 font-bold mb-1.5">
        ...
        {items.map((it, idx) => {
          ...
          return (
            <div key={idx} className="grid grid-cols-1 md:grid-cols-[2fr_70px_100px_170px_110px_28px] gap-2 mb-2 items-center border md:border-0 rounded-lg p-2 md:p-0">
              ...
              <div className="flex gap-1">
                ...
                <input type="number" step="0.01" value={it.discountValue} min={0} onChange={(e) => updateItem(idx, 'discountValue', e.target.value)} placeholder="0" className="!text-xs" />
              </div>
